function [output] = Propagation3Ddoublet(NumberOfRays,DistBetwParticles,StartDiameters)
% This function calculates the absorption for a particle doublet based on their relative overlap.
...
% Create ParticleList
% [X Y Z d absorbed]
NewDiameters = DiameterIncrease(StartDiameters,DistBetwParticles);
ParticleList = [NewDiameters/2 NewDiameters/2 NewDiameters/2 NewDiameters 0;
                NewDiameters/2+DistBetwParticles NewDiameters/2 NewDiameters/2 NewDiameters 0];
...
while ~all(cellfun(@(x)x(end,10),LaserList))
    for RayNr = 1:size(LaserList,2)
        ...
        % Steps if change in medium
            ...
            if size(Pout,1) == 2
                % Create new ray
                LaserList{end+1}(1,1:3) = Pout(2,:); %#ok<AGROW>
        ...
        if Teleported==1 || Reflected==1
            [LaserList{RayNr}(NewStep,8),InParticleList] = InParticle3D(LaserList{RayNr}(NewStep,1:3),ParticleList);
            LaserList{RayNr}(NewStep,9) = MatrixToNumber(InParticleList,decimals);
            Teleported = 0;     %#ok<NASGU>
            Reflected = 0;      %#ok<NASGU>
        end
    end
